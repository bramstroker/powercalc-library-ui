...
for (const route of [
  { path: "/", heading: "Find a power profile", name: "homepage" },
  { path: "/manufacturers/tuya", heading: "Tuya", name: "manufacturer" },
  { path: "/profiles/aeotec/zw117", heading: "Aeotec ZW117", name: "profile" },
]) {
  test(`keeps the ${route.name} within its runtime performance budgets`, async ({
    page,
    context,
    browserName,
  }, testInfo) => {
    ...
    expect(usableMs, "hydrated page on a cold slow connection").toBeLessThanOrEqual(
    ...
    if (metrics.cls > 0.1 || process.env.PERFORMANCE_DEBUG) {
      console.log(JSON.stringify(metrics.shifts));
    }
    console.log(
      `${isMobile ? "Mobile" : "Desktop"} ${route.name}: LCP ${metrics.lcp.toFixed(0)} ms, observed interaction ${metrics.inp.toFixed(0)} ms, CLS ${metrics.cls.toFixed(3)}, ${requestCount} requests, ${(scriptTransferBytes / 1024).toFixed(1)} KiB JS transferred, usable ${usableMs} ms`,
    );
    ...
    expect(requestCount, "initial page requests including dynamic chunks").toBeLessThanOrEqual(
