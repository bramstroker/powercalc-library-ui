...
import { buildSensorYaml, MY_HA_CONFIG_FLOW_URL } from "./profileSetupYaml";

const ENTITY_PLACEHOLDER = "light.my_light";

export const ProfileSetupDetails = ({
  profile,
  expanded,
}: {
  ...
  const {
    ...
    enabled: expanded && (isManualOnly || manualSetupExpanded) && profile.subProfileCount > 0,
  });
      ...
      {isManualOnly ? (
        <>
          <Alert severity="info" variant="outlined" sx={{ mb: 2 }}>
            Automatic discovery is not available for this profile. Set it up manually using one of
            the options below.
          </Alert>
          {manualSetup}
        </>
