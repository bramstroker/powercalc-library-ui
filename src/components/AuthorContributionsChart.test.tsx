...
const profile = (createdAt: string) =>
  ({ createdAt: new Date(createdAt) }) as PowerProfile;
...
describe("AuthorContributionsChart", () => {
  ...
  it("renders a bar per month between the first and last contribution", () => {
    ...
    expect(container.querySelectorAll("rect.MuiBarChart-element")).toHaveLength(
      4,
    );
  });
  ...
  it("shows a useful milestone when all contributions fall in a single month", () => {
    render(
      <AuthorContributionsChart
        profiles={[profile("2025-01-15"), profile("2025-01-20")]}
      />,
    );

    expect(
      screen.getByText("First contribution in January 2025"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("2 profiles contributed that month"),
    ).toBeInTheDocument();
  });
