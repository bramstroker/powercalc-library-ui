...
import { GithubAvatar } from "../../contributor/GithubAvatar";
...
const ManufacturerCell = ({ fullName }: { fullName: string }) => {
  ...
  return (
    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
      ...
      <Link component={RouterLink} to={manufacturerPath(manufacturer.dirName)} prefetch="intent">
        {fullName}
      </Link>
    </Box>
  );
              ...
              <TableRow key={item.name}>
                ...
                <TableCell>
                  {filterQueryParam === "author" ? (
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                      <GithubAvatar username={item.name} sx={{ width: 24, height: 24 }} />
                      <Link component={RouterLink} to={authorPath(item.name)} prefetch="intent">
                        {item.name}
                      </Link>
                    </Box>
